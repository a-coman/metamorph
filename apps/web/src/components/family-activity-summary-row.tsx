...
export function FamilyActivitySummaryRow({
  ...
  return (
    <div className="grid grid-cols-2 gap-2 pb-4 border-b border-border/60">
      {families.map((bucket) => {
        const isSelected =
          selected.kind === 'family' && selected.mrVersionId === bucket.mrVersionId;

        return (
          <div
            key={bucket.mrVersionId}
            className={cn(
          ...
          >
            {/* Filter button — selects this relation's activity feed */}
            <button
              type="button"
              onClick={() =>
                onSelect({
                  ...
                  mrVersionId: bucket.mrVersionId,
                })
              }
              className="flex flex-col gap-1 flex-1 min-w-0 px-3 py-2.5 text-left hover:bg-muted/50 transition-colors"
            >
            ...
            {/* Navigation link — opens the MR detail page */}
            <Link
