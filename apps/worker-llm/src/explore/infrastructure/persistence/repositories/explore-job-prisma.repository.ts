...
import { isTransformFamily } from '@metamorph/core';
import { exploreJobDbPayloadSchema } from '@metamorph/contracts';
...
export class ExploreJobPrismaRepository extends ExploreJobRepositoryPort {
  async findById(jobId: string): Promise<ExploreJob | null> {
    ...
    const parsedPayload = exploreJobDbPayloadSchema.safeParse(row.payload);
    if (!parsedPayload.success) {
      throw new Error(`Invalid explore job payload for ${jobId}`);
    }

    const transformFamily = parsedPayload.data.transform_family;
    if (!isTransformFamily(transformFamily)) {
      throw new Error(`Invalid transform_family in explore job ${jobId}`);
    }
    ...
    return ExploreJob.reconstitute(
      {
        ...
        pageSnapshotId: parsedPayload.data.page_snapshot_id,
        transformFamily,
